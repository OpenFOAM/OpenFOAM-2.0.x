...
defineTypeNameAndDebug(Foam::treeDataPolyMeshCell, 0);


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

Foam::treeBoundBox Foam::treeDataPolyMeshCell::calcCellBb(const label cellI) const
{
...
void Foam::treeDataPolyMeshCell::update()
{
    if (cacheBb_)
    {
...
Foam::treeDataPolyMeshCell::treeDataPolyMeshCell
(
    const bool cacheBb,
    const polyMesh& mesh,
...
Foam::pointField Foam::treeDataPolyMeshCell::shapePoints() const
{
...
bool Foam::treeDataPolyMeshCell::overlaps
(
    const label index,
...
bool Foam::treeDataPolyMeshCell::contains
(
    const label index,
    const point& sample
) const
{
    ...
    mesh_.findTetFacePt(cellLabels_[index], sample, tetFaceI, tetPtI);
    return tetFaceI != -1;
}


void Foam::treeDataPolyMeshCell::findNearest
(
...
bool Foam::treeDataPolyMeshCell::intersects
(
    const label index,
    const point& start,
    const point& end,
